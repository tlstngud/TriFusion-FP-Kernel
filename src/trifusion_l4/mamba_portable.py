...
from .vendor.utils import cos_approx, sin_approx, tanh_approx, silu, sigmoid_approx
...
def mamba3_siso_fwd_kernel(
    ...
    stride_z_head,
    stride_z_vdim,
    stride_init_ssm_state_seq,
    stride_init_ssm_state_head,
    stride_init_ssm_state_vdim,
    stride_init_ssm_state_qkdim,
    stride_init_k_state_seq,
    stride_init_k_state_head,
    stride_init_k_state_qkdim,
    stride_init_v_state_seq,
    stride_init_v_state_head,
    stride_init_v_state_vdim,
    stride_cu_seqlen,
    # Output Strides
    stride_o_batch,
    stride_o_seqlen,
    ...
    stride_o_v_head,
    stride_o_v_vdim,
    stride_ssm_states_batch,
    stride_ssm_states_head,
    stride_ssm_states_vdim,
    stride_ssm_states_qkdim,
    stride_da_cs_store_batch,
    stride_da_cs_store_head,
    stride_da_cs_store_seqlen,
    stride_da_cs_sum_store_batch,
    stride_da_cs_sum_store_head,
    stride_da_cs_sum_store_seqlen,
    stride_q_store_batch,
    stride_q_store_seqlen,
    stride_q_store_head,
    stride_q_store_qkdim,
    stride_k_store_batch,
    stride_k_store_seqlen,
    stride_k_store_head,
    stride_k_store_qkdim,
    stride_qk_store_batch,
    stride_qk_store_head,
    stride_qk_store_seqlen,
    stride_scale_store_batch,
    stride_scale_store_head,
    stride_scale_store_seqlen,
    stride_gamma_store_batch,
    stride_gamma_store_head,
    stride_gamma_store_seqlen,
    stride_final_ssm_state_seq,
    stride_final_ssm_state_head,
    stride_final_ssm_state_vdim,
    stride_final_ssm_state_qkdim,
    stride_final_k_state_seq,
    stride_final_k_state_head,
    stride_final_k_state_chunk,
    ...
    q_ptr = (
        Q + pid_batch * stride_q_batch + head_idx_qk * stride_q_head + seq_offset * stride_q_seqlen
    )
    k_ptr = (
        K + pid_batch * stride_k_batch + head_idx_qk * stride_k_head + seq_offset * stride_k_seqlen
    )
    v_ptr = V + pid_batch * stride_v_batch + pid_head * stride_v_head + seq_offset * stride_v_seqlen
    adt_ptr = (
        ADT
        + pid_batch * stride_adt_batch
        + pid_head * stride_adt_head
        + seq_offset * stride_adt_seqlen
    )
    dt_ptr = (
        DT + pid_batch * stride_dt_batch + pid_head * stride_dt_head + seq_offset * stride_dt_seqlen
    )
    trap_ptr = (
        Trap
        + pid_batch * stride_trap_batch
        + pid_head * stride_trap_head
        + seq_offset * stride_trap_seqlen
    )
    ...
    angle_ptr = (
        ...
        + seq_offset * stride_angles_seqlen
    )
    ...
    if HAS_Z:
        z_ptr = (
            Z + pid_batch * stride_z_batch + pid_head * stride_z_head + seq_offset * stride_z_seqlen
        )
    ...
    if HAS_INITIAL_STATES:
        init_ssm_state_ptr = (
            Initial_SSM_State
            + seq_idx * stride_init_ssm_state_seq
            + pid_head * stride_init_ssm_state_head
        )
        init_k_state_ptr = (
            Initial_K_State
            + seq_idx * stride_init_k_state_seq
            + pid_head * stride_init_k_state_head
        )
        init_v_state_ptr = (
            Initial_V_State
            + seq_idx * stride_init_v_state_seq
            + pid_head * stride_init_v_state_head
        )

    # Setup output pointers
    o_ptr = (
        Out + pid_batch * stride_o_batch + pid_head * stride_o_head + seq_offset * stride_o_seqlen
    )
    if STORE_SSM_STATES_ADT_OUTV:
        out_v_ptr = (
            Out_v
            + pid_batch * stride_o_v_batch
            + pid_head * stride_o_v_head
            + seq_offset * stride_o_v_seqlen
        )
        ssm_states_ptr = (
            SSM_States
            + pid_batch * stride_ssm_states_batch
            + pid_head * stride_ssm_states_head
            + chunk_offset * HEADDIM_QK * stride_ssm_states_qkdim
        )
        da_cs_store_ptr = (
            DA_CS_Store
            + pid_batch * stride_da_cs_store_batch
            + pid_head * stride_da_cs_store_head
            + seq_offset * stride_da_cs_store_seqlen
        )
        da_cs_sum_store_ptr = (
            DA_CS_SUM_Store
            + pid_batch * stride_da_cs_sum_store_batch
            + pid_head * stride_da_cs_sum_store_head
            + chunk_offset * stride_da_cs_sum_store_seqlen
        )

    q_store_ptr = (
        Q_store
        + pid_batch * stride_q_store_batch
        + pid_head * stride_q_store_head
        + seq_offset * stride_q_store_seqlen
    )
    k_store_ptr = (
        K_store
        + pid_batch * stride_k_store_batch
        + pid_head * stride_k_store_head
        + seq_offset * stride_k_store_seqlen
    )
    qk_store_ptr = (
        QK_store
        + pid_batch * stride_qk_store_batch
        + pid_head * stride_qk_store_head
        + seq_offset * stride_qk_store_seqlen
    )
    scale_store_ptr = (
        Scale_store
        + pid_batch * stride_scale_store_batch
        + pid_head * stride_scale_store_head
        + seq_offset * stride_scale_store_seqlen
    )
    gamma_store_ptr = (
        Gamma_store
        + pid_batch * stride_gamma_store_batch
        + pid_head * stride_gamma_store_head
        + seq_offset * stride_gamma_store_seqlen
    )

    if RETURN_FINAL_STATES:
        final_ssm_state_ptr = (
            Final_SSM_State
            + seq_idx * stride_final_ssm_state_seq
            + pid_head * stride_final_ssm_state_head
        )
        final_k_state_ptr = (
            Final_K_State
            + seq_idx * stride_final_k_state_seq
            + pid_head * stride_final_k_state_head
        )
    ...
    q_store_desc = tl.make_block_ptr(
        q_store_ptr,
        shape=[seqlen, headdim_qk],
    ...
    k_store_desc = tl.make_block_ptr(
        k_store_ptr,
        shape=[seqlen, headdim_qk],
    ...
    if STORE_SSM_STATES_ADT_OUTV:
        ssm_states_desc = tl.make_block_ptr(
            ssm_states_ptr,
    ...
    for chunk_idx in range(num_chunks):
        ...
        q_pre_block = tl.load(
            tl.advance(q_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
        )
        k_pre_block = tl.load(
            tl.advance(k_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
        )
        ...
        angle_block = tl.load(
            ...
            mask=(offs_seqlen[:, None] < seqlen) & (offs_hdr[None, :] < headdim_angles),
            other=0.0,
        )

        # Compute shifted gamma and scale
        dt = tl.load(
            dt_ptr + offs_seqlen * stride_dt_seqlen, mask=offs_seqlen < seqlen, other=0.0
        ).to(tl.float32)
        dt_shifted = tl.load(
            dt_ptr + (offs_seqlen + 1) * stride_dt_seqlen, mask=offs_seqlen + 1 < seqlen, other=0.0
        ).to(tl.float32)
        trap = tl.load(
            trap_ptr + offs_seqlen * stride_trap_seqlen, mask=offs_seqlen < seqlen, other=0.0
        ).to(tl.float32)
        trap = sigmoid_approx(trap)
        trap_shifted = tl.load(
            trap_ptr + (offs_seqlen + 1) * stride_trap_seqlen,
            mask=offs_seqlen + 1 < seqlen,
            other=0.0,
        ).to(tl.float32)
        ...
        tl.store(
            gamma_store_ptr + offs_seqlen * stride_gamma_store_seqlen,
            gamma,
            mask=offs_seqlen < seqlen,
        )
        tl.store(
            scale_store_ptr + offs_seqlen * stride_scale_store_seqlen,
            scale,
            mask=offs_seqlen < seqlen,
        )
        ...
        # Triton 3.3 does not accept dot with N=1; this is a row sum.
        store_qk_dot = tl.sum(q_pre_block * k_pre_block, 1).to(q_pre_block.dtype)
        store_qk_dot *= gamma
        tl.store(
            qk_store_ptr + offs_seqlen * stride_qk_store_seqlen,
            store_qk_dot,
            mask=offs_seqlen < seqlen,
        )
        ...
        if RETURN_FINAL_STATES:
            if chunk_idx == num_chunks - 1:
                tl.store(
                    final_k_state_ptr
                    + tl.arange(0, CHUNK_SIZE)[:, None] * stride_final_k_state_chunk
        ...
        tl.store(
            tl.advance(k_store_desc, [chunk_start, 0]),
            k_pre_block.to(K.dtype.element_ty),
            boundary_check=(0, 1),
        )
        ...
        tl.store(
            tl.advance(q_store_desc, [chunk_start, 0]),
            q_pre_block.to(Q.dtype.element_ty),
            boundary_check=(0, 1),
        )
    ...
    if HAS_INITIAL_STATES:
        acc_ssm_states = tl.load(
            ...
            mask=(tl.arange(0, HEADDIM_V)[:, None] < headdim_v)
            & (tl.arange(0, HEADDIM_QK)[None, :] < headdim_qk),
            other=0.0,
        ).to(tl.float32)
        ...
        input_v_state = tl.load(
            ...
            mask=tl.arange(0, HEADDIM_V) < headdim_v,
            other=0.0,
        ).to(tl.float32)
        ...
        acc_ssm_states += (
            input_v_state[:, None] * input_k_state[None, :] * dt_scalar * (1 - trap_scalar)
        )
    else:
    ...
    for chunk_idx in range(num_chunks):
        ...
        q_block = tl.load(
            tl.advance(q_store_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
        )
        k_block = tl.load(
            tl.advance(k_store_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
        )
        v_block = tl.load(
            tl.advance(v_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
        )
        if HAS_Z:
            z_block = tl.load(
                tl.advance(z_desc, [chunk_start, 0]), boundary_check=(0, 1), padding_option="zero"
            )
        ...
        if STORE_SSM_STATES_ADT_OUTV:
            tl.store(
                da_cs_store_ptr + offs_seqlen * stride_da_cs_store_seqlen,
                da_cs,
                mask=offs_seqlen < seqlen,
            )
        ...
        s_block = tl.where(
            tl.arange(0, CHUNK_SIZE)[:, None] > tl.arange(0, CHUNK_SIZE)[None, :], s_block, 0.0
        )
        ...
        if STORE_SSM_STATES_ADT_OUTV:
            tl.store(
                out_v_ptr
                + offs_seqlen[:, None] * stride_o_v_seqlen
                + tl.arange(0, HEADDIM_V)[None, :] * stride_o_v_vdim,
                acc_o,
                mask=(offs_seqlen[:, None] < seqlen)
                & (tl.arange(0, HEADDIM_V)[None, :] < headdim_v),
            )
        ...
        # Store output
        tl.store(
            tl.advance(o_desc, [chunk_start, 0]),
        ...
        if STORE_SSM_STATES_ADT_OUTV:
            tl.store(
                tl.advance(ssm_states_desc, [0, chunk_idx * headdim_qk]),
    ...
    if RETURN_FINAL_STATES:
        tl.store(
            final_ssm_state_ptr
            + tl.arange(0, HEADDIM_V)[:, None] * stride_final_ssm_state_vdim
...
def mamba3_siso_fwd(
    ...
    if Initial_States is not None:
        Init_SSM_State = (
            Init_SSM_State.contiguous() if not Init_SSM_State.is_contiguous() else Init_SSM_State
        )
        Init_K_State = (
            Init_K_State.contiguous() if not Init_K_State.is_contiguous() else Init_K_State
        )
        Init_V_State = (
            Init_V_State.contiguous() if not Init_V_State.is_contiguous() else Init_V_State
        )
    ...
    if store_states_adt_outv:
        SSM_States = torch.zeros(
            (batch, nheads, headdim_v, nchunks * headdim_qk), device=device, dtype=torch.bfloat16
        )
    ...
    if return_final_states:
        Final_SSM_State = torch.empty(
            (num_sequences, nheads, headdim_v, headdim_qk), device=device, dtype=torch.float32
        )
        Final_K_State = torch.empty(
            (num_sequences, nheads, chunk_size, headdim_qk), device=device, dtype=torch.float32
        )
    else:
    ...
    mamba3_siso_fwd_kernel[grid](
        ...
        QK_store.stride(1),
        QK_store.stride(2),
        Scale_store.stride(0),
        Scale_store.stride(1),
        Scale_store.stride(2),
        Gamma_store.stride(0),
        Gamma_store.stride(1),
        Gamma_store.stride(2),
    ...
    if return_final_states:
        if is_varlen:
            ...
            final_k = Final_K_State[
                torch.arange(num_sequences, device=device), :, last_chunk_pos, :
            ]
